...
/**
 * Repaint a custom action button on the top right of the item when the item is selected
 * @param {HTMLElement} anchor
 * @protected
 */
Item.prototype._repaintCustomActionButton = function (anchor) {
  var actionable = this.options.customAction;

  if (this.selected && actionable && !this.dom.customActionButton) {
    // create and show button
    var me = this;

    var customActionButton = document.createElement('div');

    if (this.options.rtl) {
      customActionButton.className = me.options.customAction.className + '-rtl';
    } else {
      customActionButton.className = me.options.customAction.className;
    }
    customActionButton.title = me.options.customAction.title;

    // TODO: be able to destroy the button
    new Hammer(customActionButton).on('tap', function (event) {
      event.stopPropagation();
      me.options.customAction.onClick(me);
    });

    anchor.appendChild(customActionButton);
    this.dom.customActionButton = customActionButton;
  }
  else if (!this.selected && this.dom.customActionButton) {
    // remove button
    if (this.dom.customActionButton.parentNode) {
      this.dom.customActionButton.parentNode.removeChild(this.dom.customActionButton);
    }
    this.dom.customActionButton = null;
  }
};
