...
/**
 * Repaint a edit button on the top right of the item when the item is selected
 * @param {HTMLElement} anchor
 * @protected
 */
Item.prototype._repaintEditButton = function (anchor) {
  var editable = (this.options.editable.edit ||
                  this.data.editable === true) &&
                  this.data.editable !== false;

  if (this.selected && editable && !this.dom.editButton) {
    // create and show button
    var me = this;
    ...
    if (this.options.rtl) {
      editButton.className = 'vis-edit-rtl';
    } else {
      editButton.className = 'vis-edit';
    }
    editButton.title = 'Edit this item';

    // TODO: be able to destroy the delete button
    new Hammer(editButton).on('tap', function (event) {
      event.stopPropagation();
      var itemData = me.parent.itemSet.itemsData.get(item.id); // get a clone of the data from the dataset
      me.parent.itemSet.options.onUpdate(itemData, function (itemData) {
        if (itemData) {
          me.parent.itemSet.itemsData.getDataSet().update(itemData);
        }
      });
    });
